    ...
    list_for_each_safe(pos, n, &sched_task_list) {
        ...
        /* Note: When we'll have SMP, the deallocation below will break.
            ...
            if (entry->kernel_stack_pointer && entry->vmem)
                vmm_free_pages(arch_get_kernel_pagedir(), entry->kernel_stack_pointer, (CONFIG_STACK_SIZE >> 12) + 1);
...
int sched_is_running() { return sched_initialized; }
