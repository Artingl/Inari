...
int console_write_system(const char *s, size_t sz) {
    return console_write(0, s, sz);
}

/* Tie backend device (vga/serial/vconsole) with tty */
int console_add_backend(dev_t dev, uint16_t tty_id) {
    return -1;
}

int console_free_backend(dev_t dev) {
    return -1;
}
...
int console_cleanup_proc(pid_t target) {
    return -1;
}

/* Write to tty N. Can later be read via the function below; Automatically broadcast to backends if tty0 */
...
/* Rewind, Rewind-clr, Clr, getc */
int console_ioctl(uint16_t tty_id, ...) {
    return -1;
}

/* Writes input into tty ring, which can later be read from getc ioctl */
int console_write_in_queue(uint16_t tty_id, struct console_input in) {
    return -1;
}
