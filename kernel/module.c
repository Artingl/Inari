...
static int event_handler(event_t event) {
    ...
    for (module_metadata_t *dev = (module_metadata_t *)&__start_modules; dev < (module_metadata_t *)&__stop_modules;
         dev++) {
...
void modules_cleanup() {
    for (module_metadata_t *dev = (module_metadata_t *)&__start_modules; dev < (module_metadata_t *)&__stop_modules;
        ...
        if (dev->module->flags & MODULE_FLAG_IS_LOADED) {
            event_bus_broadcast((event_t){.type = EVENT_MODULE_UNLOAD, .as = {.custom = (void *)dev->name}});
...
int modules_insmod(const char *name) {
    ...
    for (module_metadata_t *dev = (module_metadata_t *)&__start_modules; dev < (module_metadata_t *)&__stop_modules;
        ...
        if (!(dev->module->flags & MODULE_FLAG_IS_LOADED) && strcmp(dev->name, name) == 0) {
            ...
            event_bus_broadcast((event_t){.type = EVENT_MODULE_LOAD, .as = {.custom = (void *)dev->name}});
            dev->module->flags |= MODULE_FLAG_IS_LOADED;
            return ret;
        }
    }
...
int modules_ls(int idx, char *name, uintptr_t *ptr, uint32_t *flags) {
    ...
    for (module_metadata_t *dev = (module_metadata_t *)&__start_modules; dev < (module_metadata_t *)&__stop_modules;
        ...
        if (i++ >= (size_t)idx) {
            if (ptr)
                *ptr = (uintptr_t)dev->module->probe;
            if (name)
                memcpy((void *)name, (void *)dev->name, strlen(dev->name) + 1);
            if (flags)
                *flags = dev->module->flags;
            return 1;
        }
    }
