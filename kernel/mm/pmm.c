...
int pmm_init(void) {
    ...
    for (i = 0; i < mmap_length; i++) {
        ...
        if (entry->type == MULTIBOOT_MEMORY_AVAILABLE) {
            ...
            pmm_reserve_memory((struct reserved_memory){.start = addr, .end = addr + len});
        }
    }
    ...
    if (multiboot->flags & MULTIBOOT_INFO_MODS) {
        for (i = 0; i < multiboot->mods_count; i++) {
            ...
            pmm_reserve_memory((struct reserved_memory){.start = module->mod_start, .end = module->mod_end});
        }
    }
    ...
    if (multiboot->flags & MULTIBOOT_INFO_ELF_SHDR) {
        ...
        pmm_reserve_memory((struct reserved_memory){.start = (uintptr_t)shdr, .end = (uintptr_t)shdr + shdr_size});

        for (uint32_t i = 0; i < multiboot->u.elf_sec.num; i++) {
            if (shdr[i].sh_size > 0 && shdr[i].sh_addr > 0) {
                pmm_reserve_memory((struct reserved_memory){.start = (uintptr_t)shdr[i].sh_addr,
    ...
    pmm_reserve_memory(
        (struct reserved_memory){.start = 0, .end = (uintptr_t)&kern_virt_end - VIRTUAL_ADDR + PAGE_SIZE});
...
size_t pmm_usage() { return pages_used; }

size_t pmm_total() { return available_pages; }