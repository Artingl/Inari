...
static struct vmm_pool_entry *get_pool_entry(pagedir_t *pagedir) {
    ...
    if (pagedir == arch_get_kernel_pagedir())
        return &kernel_pool_entry;
...
void *vmm_alloc_kernel(size_t npages) {
    return alloc_range(&kernel_pool_entry, npages, VIRTUAL_ADDR, 0xFFFFFFFF, PAGE_PRESENT | PAGE_RW);
}
