...
int pmm_init(struct kernel_mmap_entry *mmap_list, size_t mmap_list_length)
{
    ...
    uintptr_t addr;
    struct kernel_mmap_entry *mmap_entry;

    printk("pmm: memory maps base = %p, len = %d", (unsigned int)mmap_list, mmap_list_length);
    ...
    for (i = 0; i < mmap_list_length; i++)
    {
        mmap_entry = &mmap_list[i];
        addr = align(mmap_entry->addr, PAGE_SIZE);
        len = mmap_entry->len;

        if (mmap_entry->type == KERN_MMAP_AVAILABLE && addr >= 0x100000)
        {
