...
int close(handle_t hndl) { return (int)syscall(5, (uint32_t)hndl, 0, 0, 0, 0); }

int execp(pid_t *pid, const char *path) { return syscall(6, (uint32_t)(pid), (uint32_t)(path), 0, 0, 0); }
...
int seek(handle_t hndl, size_t offset) { return syscall(8, (uint32_t)(hndl), (uint32_t)(offset), 0, 0, 0); }

int tell(handle_t hndl, size_t *offset) { return syscall(9, (uint32_t)(hndl), (uint32_t)(offset), 0, 0, 0); }

int size(handle_t hndl, size_t *size) { return syscall(10, (uint32_t)(hndl), (uint32_t)(size), 0, 0, 0); }

int get_pid(pid_t *pid) { return (int)syscall(11, (uint32_t)pid, 0, 0, 0, 0); }
...
int get_tid(tid_t *tid) { return (int)syscall(13, (uint32_t)tid, 0, 0, 0, 0); }

int kill_thread(tid_t tid) { return (int)syscall(14, (uint32_t)tid, 0, 0, 0, 0); }

int mount(dev_t dev, const char *path) { return (int)syscall(15, (uint32_t)dev, (uint32_t)path, 0, 0, 0); }

int unmount(const char *path) { return (int)syscall(16, (uint32_t)path, 0, 0, 0, 0); }
...
int waitpid(pid_t pid) { return (int)syscall(19, (uint32_t)pid, 0, 0, 0, 0); }
...
void memfree(void *vbase, size_t npages) { syscall(24, (uint32_t)vbase, (uint32_t)npages, 0, 0, 0); }
...
void memunmap(void *vbase, size_t len) { syscall(22, (uint32_t)vbase, (uint32_t)len, 0, 0, 0); }

int ioctl(handle_t hndl, unsigned long req, void *arg) {
    return (int)syscall(25, (uint32_t)hndl, (uint32_t)req, (uint32_t)arg, 0, 0);
}

int signal(pid_t pid, uint32_t signo) { return (int)syscall(26, (uint32_t)pid, (uint32_t)signo, 0, 0, 0); }

int signal_handler(proc_signal_t handler, uint32_t signo) {
    return (int)syscall(27, (uint32_t)handler, (uint32_t)signo, 0, 0, 0);
}

void sigreturn() { syscall(28, 0, 0, 0, 0, 0); }

void reboot(void) { syscall(29, 0, 0, 0, 0, 0); }

void poweroff(void) { syscall(30, 0, 0, 0, 0, 0); }

int rmmod(const char *name) { return (int)syscall(31, (uint32_t)name, 0, 0, 0, 0); }

int insmod(const char *name) { return (int)syscall(32, (uint32_t)name, 0, 0, 0, 0); }
...
int flush_hndl(handle_t hndl) { return (int)syscall(35, (uint32_t)hndl, 0, 0, 0, 0); }
...
int ipc_open(const char *name, handle_t *ipc) { return syscall(42, (uint32_t)name, (uint32_t)ipc, 0, 0, 0); }

int ipc_close(handle_t ipc) { return syscall(43, (uint32_t)ipc, 0, 0, 0, 0); }
