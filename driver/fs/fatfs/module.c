...
DSTATUS ff_disk_status(dev_t pdrv) { return RES_OK; }

DSTATUS ff_disk_initialize(dev_t pdrv) { return RES_OK; }

DRESULT ff_disk_read(dev_t pdrv,   /* Physical drive nmuber to identify the drive */
...
static int fatfs_readdir(struct vfs_mount_point *mount, const char *path, struct vfs_node *node, size_t offset) {
    ...
    if (f_opendir(&dp, mount->fs_data, path) != FR_OK)
        return -ENOENT;
    for (;;) {
        f_readdir(&dp, mount->fs_data, &fno);
        if (fno.fname[0] == 0)
            break;
        if (file_id++ < node->off - offset)
            continue;
...
static struct vfs_layer_ops fatfs_ops = {
    .open = &fatfs_open, .close = &fatfs_close, .read = &fatfs_read, .size = &fatfs_size, .readdir = &fatfs_readdir};

static struct vfs_layer fatfs_layer = {
    .name = "fatfs", .mount = &fatfs_mount_bdev, .unmount = &fatfs_unmount_bdev, .ops = &fatfs_ops};

static int fatfs_probe() { return vfs_add_layer(&fatfs_layer); }

static void fatfs_cleanup() { vfs_remove_layer(&fatfs_layer); }

module_t fatfs_module = {.probe = fatfs_probe, .cleanup = fatfs_cleanup};

module_register("fatfs", fatfs_module);
