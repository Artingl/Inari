...
static int mbr_read_blocks(struct device *bdev, uint64_t lba, void *buf, size_t nblocks) {
    ...
    if (lba > partition->end_lba - partition->start_lba)
        return -EINVAL;
...
static struct block_ops mbr_block_ops = {.read_blocks = &mbr_read_blocks, .write_blocks = &mbr_write_blocks};
...
static void register_partition_table(struct device *bdev, struct mbr_header header, struct mbr_partition_table tbl) {
    ...
    register_blkdev(MBR_DRIVER, &mbr_block_ops, (partition->end_lba - partition->start_lba) * bdev->group->block_size,
                    (void *)partition, NULL);

#ifdef CONFIG_DEBUG
    kprintf("mbr: found partition on dev:blk_%s%d; size %uKiB", bdev->group->name, DEVID(partition->disk_bdev),
...
module_register("mbr", mbr_module);
