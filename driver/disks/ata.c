...
static struct block_ops ata_block_ops = {.read_blocks = &ata_read_blocks, .write_blocks = &ata_write_blocks};
...
static int ata_identify(uint8_t drive_id) {
    ...
    kprintf("ata: drive %s on %u[%u] %dKB; dma=%u, lba48=%u", drive->model, drive->drive, drive->controller,
            drive->size / 2, drive->supports_dma, drive->supports_lba48);

    /* We support only pio mode for now */
    ata_pio_init(drive);

    register_blkdev(ATA_DRIVER, &ata_block_ops, drive->size * 512, (void *)drive, NULL);
    return 0;
}
...
static void ata_cleanup() { unregister_blkdev_group(ATA_DRIVER); }

module_t ata_module = {.probe = ata_probe, .cleanup = ata_cleanup};

module_register("ata", ata_module);
