...
static int initrd_read(struct device *bdev, uint64_t lba, void *buf, size_t nblocks) {
    if (!bdev || !bdev->driver_data || !buf)
        return -EINVAL;
    multiboot_module_t *module = (multiboot_module_t *)bdev->driver_data;
    if ((lba * RAMDISK_BLOCK_SIZE + nblocks * RAMDISK_BLOCK_SIZE) >= bdev->size)
        return -EINVAL;
...
static struct block_ops ops = {.read_blocks = &initrd_read};
...
module_t ramdisk_module = {.probe = ramdisk_probe, .cleanup = ramdisk_cleanup};

module_register("ramdisk_multiboot", ramdisk_module);
