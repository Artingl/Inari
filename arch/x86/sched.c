...
int arch_sched_load(struct thread *task) {
    ...
    if (!task->kernel_stack_pointer) {
        ...
        if (is_kernel_pagedir) {
            esp -= 4;
            *(((uint32_t *)esp)) = ((struct x86_regs32 *)frame->registers.base)->eflags;
            esp -= 4;
            *(((uint32_t *)esp)) = 0x08;
            esp -= 4;
            *(((uint32_t *)esp)) = (uint32_t)&sched_thread_preentry;
        } else {
            ...
            esp -= 4;
            *(((uint32_t *)esp)) = 0x23; // ss
            esp -= 4;
            *(((uint32_t *)esp)) = (uint32_t)task->thread_stack_pointer + (uint32_t)CONFIG_STACK_SIZE; // useresp
            esp -= 4;
            *(((uint32_t *)esp)) = 0x202;
            esp -= 4;
            *(((uint32_t *)esp)) = 0x1b;
            esp -= 4;
            *(((uint32_t *)esp)) = (uint32_t)task->entrypoint;
        }
        ...
        esp -= 4;
        *(((uint32_t *)esp)) = segments; // ds
        esp -= 4;
        *(((uint32_t *)esp)) = segments; // es
        esp -= 4;
        *(((uint32_t *)esp)) = segments; // fs
        esp -= 4;
        *(((uint32_t *)esp)) = segments; // gs

        task->saved_stack = (void *)esp;
    } else if (task->proc_data && !is_kernel_pagedir && task->proc_data->pending_signal != 0 &&
               (regs_frame->cs & 0x03) == 0x03) {
