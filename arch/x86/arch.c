...
void _arch_disable_int(void) { __asm__ volatile("cli"); }
...
void _arch_enable_int(void) { __asm__ volatile("sti"); }
...
void x86_outl(uint16_t port, uint32_t val) { __asm__ volatile("outl %0, %1" : : "a"(val), "Nd"(port)); }

void x86_outb(uint16_t port, uint8_t val) { __asm__ volatile("outb %0, %1" : : "a"(val), "Nd"(port)); }

void x86_outw(uint16_t port, uint16_t val) { __asm__ volatile("outw %w0, %w1" : : "a"(val), "Nd"(port)); }

void x86_outsw(unsigned short int __port, const void *__addr, unsigned long int __count) {
...
void x86_set_msr(uint32_t msr, uint32_t lo, uint32_t hi) { __asm__ volatile("wrmsr" : : "a"(lo), "d"(hi), "c"(msr)); }

void x86_rdtsc(uint32_t *lo, uint32_t *hi) { __asm__ volatile("rdtsc" : "=a"(*lo), "=d"(*hi)); }

void x86_cpuid(uint32_t leah, uint32_t *eax, uint32_t *ebx, uint32_t *ecx, uint32_t *edx) {
    __asm__ volatile("cpuid" : "=a"(*eax), "=b"(*ebx), "=c"(*ecx), "=d"(*edx) : "0"(leah));
}
