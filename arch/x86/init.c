...
_lo_text static struct x86_page_table *x86_alloc_table(uintptr_t offset) {
    struct x86_page_table *table_pool =
        (struct x86_page_table *)ALIGN((uintptr_t)&_arch_pagetable_area_start, PAGE_SIZE);

    if (!(directory.tables_phys[offset] & TABLE_PRESENT)) {
        struct x86_page_table *table =
            (struct x86_page_table *)ALIGN((uintptr_t)&table_pool[table_pool_offset++], PAGE_SIZE);
...
_lo_text static void x86_map_section(uintptr_t v_start, uintptr_t v_end, uintptr_t p_start) {
