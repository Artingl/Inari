...
{
    extern char _arch_paging_pool;
    struct page_table *table_pool = (struct page_table*)ALIGN((uintptr_t)&_arch_paging_pool, KERN_PAGE_SIZE);

    if (!(tables_phys[offset] & KERN_TABLE_PRESENT))
    {
        struct page_table *table = (struct page_table*)&table_pool[table_pool_offset++];
        tables_phys[offset] = ((uintptr_t)table) | 3; // present, RW
    }
    return (struct page_table *)(tables_phys[offset] >> 3 << 3);
}
