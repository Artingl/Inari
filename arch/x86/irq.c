...
#define DECL_DIRECT(n, flags)                                                                                          \
    extern void _arch_irq##n(void);                                                                                    \
    x86_cpu_install_idt(core->core_id, (unsigned)_arch_irq##n, n, 0x08, flags)
#define DECL_IRQ(n)                                                                                                    \
    extern void _arch_irq##n(void);                                                                                    \
    x86_cpu_install_idt(core->core_id, (unsigned)_arch_irq##n, 32 + n, 0x08, IDT_PRESENT | IDT_INT32_GATE)
...
void x86_irq_handler(struct x86_regs32 *regs) {
    ...
    } else if (regs->int_no == X86_SWI_RESCHEDULE)
        irq = SWI_RESCHEDULE;
    else if (regs->int_no == X86_SWI_SYSCALL) {
        regs->ebx = syscall_handle(regs->ebx, (void *)regs->ecx, (void *)regs->edx, (void *)regs->esi,
                                   (void *)regs->edi, (void *)regs->ebp);
        irq = SWI_SYSCALL;
    } else
        irq -= X86_IRQ_OFFSET;

    interrupt_dispatch(
        (struct interrupt_frame){.int_no = irq, .registers = {.base = regs, .size = sizeof(struct x86_regs32)}});
