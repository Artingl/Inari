...
int x86_acpi_init(void) {
    ...
    pmm_reserve_memory((struct reserved_memory){.start = (uintptr_t)sdp, .end = (uintptr_t)sdp + PAGE_SIZE});
    pmm_reserve_memory((struct reserved_memory){.start = (uintptr_t)root_sdt, .end = (uintptr_t)root_sdt + PAGE_SIZE});
    ...
    ACPI_ITERATE(idx, base, {
        pmm_reserve_memory((struct reserved_memory){.start = (uintptr_t)base, .end = (uintptr_t)base + PAGE_SIZE});
    })
...
int x86_acpi_load_madt(struct x86_cpu *cpus_pool) {
    ...
    ACPI_ITERATE(idx, pointer, {
        if (memcmp(pointer->signature, "APIC", 4) == 0) {
            ...
            /* Iterate thru all MADT entries to get all APICs on the system */
            for (i = ((uint8_t *)(madt)) + 0x2c; i < ((uint8_t *)(madt)) + madt->header.length + 0x2c;) {
                ...
                if (entry->entry_type == APIC_LOCAL_PROCESSOR) {
                    ...
                    kprintf("acpi: io/apic ISO: bus = 0x%x, irq = 0x%x, gsi = 0x%x, flags = 0x%x", int_overrd->bus,
                            int_overrd->irq, int_overrd->gsi, int_overrd->flags);
#endif
                }
