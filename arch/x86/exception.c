...
#define DECL_EXCP(n)                                                                                                   \
    extern void _arch_excp##n(void);                                                                                   \
    x86_cpu_install_idt(core->core_id, (unsigned)_arch_excp##n, n, 0x08, IDT_PRESENT | IDT_INT32_GATE)
...
static const char *retrieve_symbol(uintptr_t base) {
    ...
    if (ksyms.symtab != NULL && ksyms.strtab != NULL) {
        for (i = 0; i < ksyms.sym_count; i++) {
            if (ELF32_ST_TYPE(ksyms.symtab[i].st_info) != STT_FUNC)
                continue;
...
void x86_exception_handler(struct x86_regs32 *regs) {
    ...
    if (sched_current_thread(&tid) == 0 && sched_get_thread(tid, &th) == 0) {
        ...
        kprintf("arch: Exception %s (0x%x); eip=0x%x (%s); esp=0x%x", exceptionstr[regs->int_no], regs->err_code,
                regs->eip, retrieve_symbol(regs->eip), regs->esp);
        if (th->proc_data) {
    ...
    interrupt_dispatch((struct interrupt_frame){.int_no = SWI_RESCHEDULE,
                                                .registers = {.base = regs, .size = sizeof(struct x86_regs32)}});
