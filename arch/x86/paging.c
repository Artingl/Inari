...
static inline void flush_tlb(unsigned long addr) { __asm__ volatile("invlpg (%0)" ::"r"(addr) : "memory"); }
...
pagedir_t *arch_get_pagedir(void) { return (pagedir_t *)x86_current_dir; }
...
pagedir_t *arch_fork_pagedir(void) {
    size_t i;
    struct x86_paging_directory_usr *new_dir =
        (struct x86_paging_directory_usr *)vmm_alloc_kernel((sizeof(struct x86_paging_directory_usr) >> 12) + 1);
    ...
    for (i = 0; i < 1024; i++) {
        if (i >= 768 || i < 4) {
            ...
            new_dir->dir.tables_phys[i] =
                (uintptr_t)arch_virt_to_phys(x86_kernel_dir, (void *)new_dir->dir.tables_virt[i]) | TABLE_PRESENT |
                TABLE_RW | TABLE_USR;
        }
    }
...
pagedir_t *arch_get_kernel_pagedir() { return (pagedir_t *)x86_kernel_dir; }
