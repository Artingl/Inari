...
int _arch_is_cpu_initalized(void) { return is_ready; }

void x86_cpu_ack_irq(uint8_t irq) { x86_pic_acknowledge(irq); }

uint32_t x86_cpu_features_ecx(void) { return features_ecx; }

uint32_t x86_cpu_features_edx(void) { return features_edx; }

const char *x86_cpu_vendor(void) { return (const char *)&s_cpu_vendor[0]; }

const char *x86_cpu_model(void) { return (const char *)&s_cpu_model[0]; }
